(function(){
  ...
  function twistsSectionHTML(c, editable){
    ...
    if(twists.length){
      out += `<div class="twist-grid">${twists.map((tw,i)=>h`<div class="twist-item"><span class="twist-num">${i+1}</span><span class="twist-text">${escapeHtml(tw)}</span>${editable?`<button type="button" class="remove-x" data-action="remove-twist" data-index="${i}" title="Remove">${icon('x')}</button>`:''}</div>`).join('')}</div>`;
    } else if(!editable){
