(function(){
  ...
  async function setCustomCover(url){
    const c = activeCase;
    if(!c) return;
    const patch = { coverUrl:url, coverManual:true, coverSource:'manual', coverChecked:true, updatedAt:new Date().toISOString() };
  ...
  function caseHTML(){
    ...
    out += h`<div class="case-header">
      <div style="display:flex;flex-direction:column;gap:8px;width:150px;flex:none">
        ...
        ${coverEditOpen ? h`
          <form data-form="custom-cover" style="display:flex;flex-direction:column;gap:6px">
            <input class="input" name="url" type="url" placeholder="Paste an image URL" required style="font-size:13px;padding:8px 10px" value="${escapeHtml(c.coverUrl||'')}">
            <div style="display:flex;gap:6px">
              <button class="btn btn-gold" type="submit" style="flex:1;padding:6px 8px;font-size:13px">Save</button>
              <button class="btn btn-ghost" type="button" data-action="toggle-cover-edit" style="flex:none;padding:6px 8px;font-size:13px">Cancel</button>
            </div>
          </form>
        ` : h`
  ...
  async function handleAction(action, t, e){
    ...
    if(action==='toggle-cover-edit'){ coverEditOpen = !coverEditOpen; render(); return; }
    if(action==='reset-cover'){ await resetCover(); return; }
    if(action==='delete-case-tile'){ modal={type:'confirm', kind:'delete-case-tile', id:t.dataset.id, title:'Delete this case?', body:'This permanently removes the case file and everything on it. This can\'t be undone.', word:'DELETE', confirmLabel:'Delete case'}; render(); return; }
  ...
  async function handleSubmit(form){
    ...
    if(type==='custom-cover'){
      const url=(fd.get('url')||'').toString().trim();
      if(!url) return;
      await setCustomCover(url);
      return;
    }
