(function(){
  ...
  function summaryHTML(){
    ...
    const twistResults = c.twistResults||[];
    ...
    if(twists.length){
      out += `<ul class="summary-list">${twists.map((tw,i)=>{
        const outcome = closed ? (twistResults[i] ? ' &mdash; came true' : ' &mdash; didn\'t happen') : '';
        return `<li>${escapeHtml(tw)}${outcome}</li>`;
      }).join('')}</ul>`;
    } else {
    ...
    if(closed && c.resolution){
      ...
      <ul class="summary-list">
        ...
        <li>Correct predictions: ${v.stats.totalTwists ? v.stats.correctPredictions+'/'+v.stats.totalTwists : '—'}</li>
  ...
  function confirmModalHTML(opts){
    if(!opts.word){
      return h`<div class="card modal">
        ...
        <div class="btn-row"><button type="button" class="btn btn-danger" data-action="confirm-simple">${escapeHtml(opts.confirmLabel)}</button><button class="btn btn-ghost" type="button" data-action="close-modal">Cancel</button></div>
      </div>`;
    }
    return h`<div class="card modal">
      <h3>${icon('warn')} ${escapeHtml(opts.title)}</h3>
  ...
  async function handleAction(action, t, e){
    ...
    if(action==='delete-case'){ modal={type:'confirm', kind:'delete-case', title:'Are you sure?', body:'This permanently removes the case file and everything on it. This can\'t be undone.', confirmLabel:'Delete case'}; render(); return; }
    ...
    if(action==='delete-case-tile'){ modal={type:'confirm', kind:'delete-case-tile', id:t.dataset.id, title:'Are you sure?', body:'This permanently removes the case file and everything on it. This can\'t be undone.', confirmLabel:'Delete case'}; render(); return; }
  ...
  async function finishWizard(finalName, finalTheory){
    ...
    const data = {
      ...
      resolution:null,
      plotTheory:wizard.plotTheory||'', twists:[], twistResults:[], journal:[], rating:0,
      public:false,
