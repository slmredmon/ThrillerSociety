(function(){
  ...
  function coverHTML(title,author,opts){
    ...
    return h`<div class="cover" style="background:${st.bg};color:${st.fg}">
      ${imgTag}
      <div class="cover-icon">${icon('fingerprint','')}</div>
      <div>
  ...
  async function fetchRealCover(title, author){
    ...
    try{
      const qParts = ['intitle:'+t];
      if(author) qParts.push('inauthor:'+author);
      const url = 'https://www.googleapis.com/books/v1/volumes?maxResults=1&q='+encodeURIComponent(qParts.join('+'));
    ...
    try{
      let olUrl = 'https://openlibrary.org/search.json?limit=1&fields=cover_i&title='+encodeURIComponent(t);
      if(author) olUrl += '&author='+encodeURIComponent(author);
      const res2 = await fetch(olUrl);
      if(res2.ok){
        const data2 = await res2.json();
        const doc = data2.docs && data2.docs[0];
        if(doc && doc.cover_i) return { url:'https://covers.openlibrary.org/b/id/'+doc.cover_i+'-L.jpg', source:'openlibrary' };
      }
  ...
  function ensureCoverForCase(c){
    if(!c || !c.id || c.coverChecked || coverLookupsInFlight.has(c.id)) return;
    c.coverChecked = true; // optimistic, so this case isn't queued again this session
    coverLookupsInFlight.add(c.id);
    fetchRealCover(c.book && c.book.title, c.book && c.book.author).then(result=>{
      const patch = { coverChecked:true, coverUrl: result?result.url:null, coverSource: result?result.source:null };
      return DAL.updateCase(c.id, patch).then(()=>{
        Object.assign(c, patch);
        if(activeCase && activeCase.id===c.id) Object.assign(activeCase, patch);
        render();
      });
    }).catch(()=>{ /* leave generated cover in place */ })
  ...
  function buildVerdict(kase, actualCulprit){
    ...
    const correctPredictions = (kase.twistResults||[]).filter(Boolean).length;
    ...
    if(totalTwists>0){
      if(correctPredictions===totalTwists){
        strengths.push(`Every twist you called (${totalTwists} for ${totalTwists}) came true &mdash; you saw the shape of this book coming.`);
      } else if(correctPredictions>0){
        strengths.push(`You called ${correctPredictions} of your ${totalTwists} predicted twists correctly.`);
      } else {
        weaknesses.push(`None of your ${totalTwists} predicted twist${totalTwists===1?'':'s'} panned out &mdash; the book zigged where you zagged.`);
      }
    }
    ...
    return {
      ...
      stats:{ totalConsidered, avgTrust: avgTrust? avgTrust.toFixed(1):'—', stayedWithPrime: !!stayedWithPrime,
        cluesFound, liesDetected, connectionsMade, correctPredictions, totalTwists }
    };
  }
  ...
  function topbarHTML(){
    ...
    return h`<div class="topbar">
      <div class="brand" data-action="go-dashboard">
        ...
        <div class="brand-text"><b>Thriller Society</b><span>Don't just read it. Investigate.</span></div>
      </div>
      ${(profile && !gated) ? h`<div class="top-actions">
        ...
        ${view!=='wizard' ? `<button class="icon-btn" data-action="start-wizard" title="Start a new case">${icon('plus')}</button>` : ''}
  ...
  function dashboardHTML(){
    ...
    if(cases.length>0){
      body += h`<div class="hero-stats">
        <div class="stat-row" style="margin-top:0">
          ...
          <div class="stat-chip"><b>${escaped.length}</b><span>escaped</span></div>
          <div class="stat-chip"><b>${solveRate===null?'—':solveRate+'%'}</b><span>solve rate</span></div>
        </div>
        ${closed.length ? h`<div class="outcome-bar" role="img" aria-label="${solved.length} solved, ${escaped.length} escaped, ${active.length} in progress">
        ...
        <div class="outcome-legend">
          ...
          <div class="outcome-legend-item"><span class="dot" style="background:var(--warn)"></span>Escaped <b>${escaped.length}</b></div>
    ...
    if(cases.length===0){
      ...
      body += h`<div class="section-head"><h2>Active Cases <span class="stamp">(${active.length})</span></h2>${active.length>0?`<button class="btn btn-primary" data-action="start-wizard">${icon('plus')} New case</button>`:''}</div>`;
      if(active.length===0){
        body += `<div class="card"><div class="empty-state"><p>No open cases — every book you've read has been closed out.</p><div class="btn-row" style="justify-content:center"><button class="btn btn-primary" data-action="start-wizard">Start a new case</button></div></div></div>`;
      } else {
  ...
  function statusBadge(c){
    if(c.status==='closed'){
      return c.resolution&&c.resolution.solved
        ...
        : `<span class="badge badge-escaped">${icon('ghost')} escaped</span>`;
    }
        ...
        <div class="case-tile-body">
          <div class="tile-progress-row">
            ...
            <span class="mini-checkbox ${solved?'is-solved':'is-escaped'}">
              ...
              <span>${solved?'Solved':'Escaped'}</span>
            </span>
          </div>
        </div>
      </div>`;
  ...
  /* ============================== WIZARD ============================== */
  function newWizard(){
    return { step:1, title:'', author:'', progress:0, primaryName:'', primaryTheory:'',
      others:[], draftName:'', draftTheory:'', draftTrust:5, finalName:'', finalTheory:'' };
  }
 
  function wizardStepCount(){ return wizard.progress>=50 ? 6 : 5; }
  ...
  function wizardHTML(){
    ...
    else if(wizard.step===4) inner=wizardStep4();
    else if(wizard.step===5) inner=wizardStep5();
    else if(wizard.step===6) inner=wizardStep6();
  ...
  function wizardStep3(){
    ...
    <div class="wizard-q">Who is your number one suspect?</div>
    <form data-form="w3">
      <div class="field"><label for="w-primary-name">Name</label><input class="input" id="w-primary-name" name="name" required value="${escapeHtml(wizard.primaryName)}" placeholder="e.g. The victim's business partner"></div>
      <div class="btn-row"><button class="btn btn-primary" type="submit">Continue</button><button class="btn btn-ghost" type="button" data-action="w-back">Back</button></div>
    </form>`;
  }
 
  function wizardStep4(){
    return h`<div class="wizard-eyebrow">Step 4 of ${wizardStepCount()}</div>
    <div class="wizard-q">Why do you suspect ${escapeHtml(wizard.primaryName)}?</div>
    <form data-form="w4">
      <div class="field"><label for="w-primary-theory">Your theory</label><textarea class="textarea input" id="w-primary-theory" name="theory" required placeholder="Lay out the evidence...">${escapeHtml(wizard.primaryTheory)}</textarea></div>
      <div class="btn-row"><button class="btn btn-primary" type="submit">Continue</button><button class="btn btn-ghost" type="button" data-action="w-back">Back</button></div>
    </form>`;
  }
  ...
  function wizardStep5(){
    ...
    return h`<div class="wizard-eyebrow">Step 5 of ${wizardStepCount()}</div>
    <div class="wizard-q">Who else is on your suspect list?</div>
    ...
    <form data-form="w5add">
      <div class="field"><label for="w-o-name">Suspect name</label><input class="input" id="w-o-name" name="name" placeholder="e.g. The neighbor" required></div>
      <div class="field"><label for="w-o-theory">Their theory</label><textarea class="textarea input" id="w-o-theory" name="theory" required placeholder="Why do they make the list?"></textarea></div>
      <div class="field">
        <label>Trust — how sure are you? (1&ndash;10)</label>
        <div class="range-row"><input type="range" min="1" max="10" value="5" id="w-o-trust" name="trust"><span class="trust-value" id="w-o-trust-val">5</span></div>
      </div>
      <button class="btn btn-gold" type="submit">${icon('plus')} Add suspect</button>
    </form>
    <div class="btn-row" style="margin-top:20px">
      <button class="btn btn-primary" data-action="w-finish">${wizard.progress>=50?'Continue to final suspect':'Finish setup'}</button>
      <button class="btn btn-ghost" data-action="w-back">Back</button>
    </div>`;
  }
  ...
  function wizardReviewHTML(){
    ...
    if(isExisting){
      out += plotTheorySectionHTML(c, false);
      out += twistsSectionHTML(c, false);
      out += journalSectionHTML(c, false);
    } else {
      out += h`<div class="suspects-head" style="margin-top:38px"><h2>${icon('ghost')} What's Really Going On, Predicted Twists &amp; Clues Found</h2></div>
      <div class="theory-box empty">Nothing to review yet &mdash; you'll be able to add a working theory, predicted twists, and a clues journal once this case is created.</div>`;
    }
    return out;
  }
  ...
  function wizardStep6(){
    ...
    let out = h`<div class="wizard-eyebrow">Step 6 of ${wizardStepCount()} &middot; Review &amp; final answer</div>
  ...
  function caseHTML(){
    ...
    let out = h`<button class="btn btn-ghost" data-action="go-dashboard" style="margin-bottom:16px">${icon('back')} All cases</button>`;
    ...
    out += plotTheorySectionHTML(c, !locked && !closed);
    ...
    if(c.finalSuspect){
      ...
      out += h`<div class="settings-row" style="margin-top:16px">
        <div class="settings-row-text"><b>${icon('link')} Community Board</b><span>Make your final suspect and theory visible to other readers of this book, so you can compare notes.</span></div>
    ...
    if(closed){
      ...
      <div class="stat-row">
        ...
        <div class="stat-chip"><b>${v.stats.totalTwists ? v.stats.correctPredictions+'/'+v.stats.totalTwists : '—'}</b><span>correct predictions</span></div>
      </div>`;
    }
  ...
  function twistsSectionHTML(c, editable){
    ...
    if(twists.length){
      out += `<div class="twist-grid">${twists.map((tw,i)=>{
        const outcome = closed ? (results[i] ? `<span class="badge badge-solved" style="flex:none">${icon('check')} came true</span>` : `<span class="badge badge-escaped" style="flex:none">${icon('x')} didn't happen</span>`) : '';
        return h`<div class="twist-item"><span class="twist-num">${i+1}</span><span class="twist-text">${escapeHtml(tw)}</span>${outcome}${editable?`<button type="button" class="remove-x" data-action="remove-twist" data-index="${i}" title="Remove">${icon('x')}</button>`:''}</div>`;
      }).join('')}</div>`;
    } else if(!editable){
  ...
  function resolveModalHTML(){
    const twists = (activeCase && activeCase.twists) || [];
    return h`<div class="card modal">
      ...
      <form data-form="resolve">
        ${twists.length ? h`<div class="field">
          <label>Which of your predicted twists came true?</label>
          <div style="display:flex;flex-direction:column;gap:10px;margin-top:6px">
            ${twists.map((tw,i)=>h`<label style="display:flex;align-items:flex-start;gap:10px;font-size:15.5px;color:var(--text-dim);font-family:var(--font-type);line-height:1.4">
              <input type="checkbox" name="twistCorrect" value="${i}" style="margin-top:4px;accent-color:var(--accent-2)">
  ...
  async function handleAction(action, t, e){
    ...
    if(action==='start-wizard'){ wizard=newWizard(); view='wizard'; render(); return; }
    ...
    if(action==='w-back'){
      if(wizard.mode==='lock-existing'){
        if(wizard.step<=5){ wizard=null; view='case'; render(); return; }
    ...
    if(action==='w-progress'){ wizard.progress = Number(t.dataset.val); wizard.progressChosen=true; render(); return; }
    if(action==='w-next'){ wizard.step=3; render(); return; }
    if(action==='w-remove-other'){ wizard.others.splice(Number(t.dataset.idx),1); render(); return; }
    ...
    if(action==='delete-case'){ modal={type:'confirm', kind:'delete-case', title:'Delete this case?', body:'This permanently removes the case file and everything on it. This can\'t be undone.', word:'DELETE', confirmLabel:'Delete case'}; render(); return; }
  ...
  async function handleSubmit(form){
    ...
    if(type==='w3'){
      ...
      if(!wizard.primaryName) return;
      wizard.step=4; render(); return;
    }
    if(type==='w4'){
      wizard.primaryTheory=(fd.get('theory')||'').toString().trim();
      if(!wizard.primaryTheory) return;
      wizard.step=5; render(); return;
    }
    if(type==='w5add'){
      const name=(fd.get('name')||'').toString().trim();
      ...
      wizard.others.push({name,theory,trust});
      render(); return;
    }
    if(type==='w6'){
    ...
    if(type==='resolve'){
      ...
      if(!culprit) return;
      const correctTwistIndices = fd.getAll('twistCorrect').map(Number);
      await resolveCase(culprit, correctTwistIndices);
      return;
    }
    if(type==='add-suspect'){
      const name=(fd.get('name')||'').toString().trim();
  ...
  async function finishWizard(finalName, finalTheory){
    // Step-5 "Continue"/"Finish setup" click (no finalName yet): if we're past
    // the 50% mark, detour through step 6 to collect the final suspect first.
    if(!finalName && wizard.progress>=50 && wizard.step<6){
      wizard.step=6; render(); return;
    }
    ...
    const data = {
      ...
      resolution:null,
      plotTheory:'', twists:[], twistResults:[], journal:[],
      public:false,
  ...
  function openFinalLockFlow(){
    const c = activeCase;
    wizard = {mode:'lock-existing', step:6, progress:50, primaryName:c.primary?c.primary.name:'', others:(c.suspects||[]).slice(), finalName:'', finalTheory:''};
    view='wizard'; render();
  }
  ...
  function communityHTML(){
    ...
    const others = communityTheories.filter(e=> !(e.uid===mine && e.caseId===c.id));
 
    let out = h`<button class="btn btn-ghost" data-action="back-to-case" style="margin-bottom:16px">${icon('back')} Back to case</button>
    <div class="suspects-head"><h2>${icon('book')} Community Board</h2></div>
    ...
    if(!others.length){
      out += `<div class="theory-box empty">No one else has made their final theory public for this book yet. Be the first &mdash; toggle "Public" on your case page once you've locked in.</div>`;
    } else {
      out += `<div class="suspect-grid">${others.map(e=>h`<div class="suspect-card">
        <span class="suspect-tag">${escapeHtml(e.displayName||'A detective')}</span>
  ...
  async function resolveCase(culprit, correctTwistIndices){
    const c = activeCase;
    const solved = c.finalSuspect ? namesMatch(c.finalSuspect.name, culprit) : false;
    const twists = c.twists || [];
    const twistResults = twists.map((_,i)=> (correctTwistIndices||[]).includes(i));
    const patch = { status:'closed', progress:100, resolution:{actualCulprit:culprit, solved, closedAt:new Date().toISOString()}, twistResults, updatedAt:new Date().toISOString() };
    await DAL.updateCase(c.id, patch);
    activeCase = Object.assign({}, c, patch);
